...
Return<int32_t> MtkPerf::perfLockAcquire(int32_t pl_handle, uint32_t duration,
                                         const hidl_vec<int32_t>& /* boostsList */,
                                         int32_t reserved) {
    LOG(INFO) << "perfLockAcquire pl_handle: " << pl_handle << " duration: " << duration
              << " reserved: " << reserved;
    return pl_handle;
}

Return<void> MtkPerf::perfLockRelease(int32_t pl_handle, int32_t reserved) {
    LOG(INFO) << "perfLockRelease pl_handle: " << pl_handle << " reserved: " << reserved;
    return Void();
}
...
Return<int32_t> MtkPerf::perfCusLockHint(int32_t hint, uint32_t duration) {
    LOG(INFO) << "perfCusLockHint hint: " << hint << " duraton: " << duration;
    return 233;
}
...
Return<int32_t> MtkPerf::perfLockReleaseSync(int32_t pl_handle, int32_t reserved) {
    LOG(INFO) << "perfLockReleaseSync pl_handle: " << pl_handle << " reserved: " << reserved;
    return 0;
}
